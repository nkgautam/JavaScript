class Wallet
    ...
    refresh_contacts: ->
        alert('refresh_contacts function is deprecated. Use refresh_contacts2')

    # added by vinay and passed wallet_api as a parameter which is WalletAPI
    refresh_contacts2: (wallet_api) ->
...
#                    deferred.resolve(acct)
    ...
    refresh_transactions: () ->
        ...
        refresh_asset_records_promise.then =>
            go_back_to_block = 0 #if @transactions_last_block > 20 then @transactions_last_block else 0
    ...
    ngbazaar: (action, param1, param2, param3) ->
        @rpc.request('wallet_ngBazaar', [action, param1, param2, param3]).then (response) =>
          response.result
